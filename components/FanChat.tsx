...
const FanChat: React.FC<Props> = ({ initialConcept, isSidebarMode = false }) => {
  ...
  if (!profile) {
    return (
      <div className={`w-full glass rounded-[3rem] p-10 border border-[#D4AF37]/20 text-center flex flex-col justify-center items-center space-y-10 animate-in fade-in duration-1000 shadow-2xl ${isSidebarMode ? 'min-h-[600px] bg-neutral-900/10' : 'max-w-4xl mx-auto py-32'}`}>
  ...
  return (
    <div className={`w-full flex flex-col glass rounded-[3rem] overflow-hidden border border-neutral-800 shadow-[0_60px_120px_rgba(0,0,0,0.6)] animate-in slide-in-from-right-12 duration-1000 ${isSidebarMode ? 'h-[800px]' : 'max-w-5xl mx-auto h-[800px]'}`}>
      
      {/* Dynamic Header */}
      <div className="px-6 py-6 border-b border-neutral-800/60 bg-neutral-950/90 backdrop-blur-3xl flex justify-between items-center z-20 shadow-xl">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="w-12 h-12 rounded-2xl border border-[#D4AF37]/40 bg-neutral-900 p-0.5 overflow-hidden shadow-2xl ring-2 ring-black/50">
               <img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${profile.avatarSeed}`} alt="Avatar" className="w-full h-full rounded-[14px]" />
            </div>
            <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-green-500 border-2 border-black rounded-full animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.6)]" />
          </div>
          <div className="min-w-0">
            <h4 className="font-cinzel font-bold text-sm text-white tracking-[0.2em] uppercase truncate drop-shadow-md">{profile.name}</h4>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[8px] text-[#D4AF37] font-black tracking-[0.3em] uppercase opacity-90 drop-shadow-sm">ONLINE</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <button 
            ...
            disabled={isTyping}
            className="p-2.5 rounded-xl bg-neutral-900 border border-neutral-800 text-neutral-500 hover:text-[#D4AF37] hover:border-[#D4AF37]/40 transition-all disabled:opacity-20 group"
          >
      ...
      {/* Messaging Chamber */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth bg-neutral-950/30 custom-scrollbar">
        {messages.map((m) => (
          <div key={m.id} className={`flex flex-col ${m.sender === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-4 duration-500`}>
            <div 
              className={`max-w-[90%] p-4 rounded-2xl text-[11px] leading-relaxed shadow-2xl transition-all duration-500 group border ${
                m.sender === 'user' 
                  ? 'bg-gradient-to-br from-[#D4AF37] to-[#8B7326] text-black rounded-tr-none font-bold border-[#D4AF37]/20 shadow-[0_10px_20px_rgba(212,175,55,0.1)]' 
                  : 'bg-neutral-900/90 text-neutral-200 border-neutral-800 rounded-tl-none font-light backdrop-blur-xl'
              }`}
            >
              {m.sender === 'character' && (
                <span className="block text-[7px] font-black text-[#D4AF37] mb-2 uppercase tracking-[0.4em] opacity-90">
                  {profile.name}
                </span>
              )}
              <p className="tracking-wide leading-relaxed">{m.text}</p>
              <div className={`flex items-center justify-end gap-2 mt-3 text-[6px] font-black tracking-widest ${m.sender === 'user' ? 'text-black/50' : 'text-neutral-600'}`}>
                {m.timestamp}
        ...
        {isTyping && (
          <div className="flex flex-col items-start animate-in fade-in duration-300">
            <div className="p-4 rounded-xl bg-neutral-900/60 border border-neutral-800 rounded-tl-none flex gap-1.5 items-center">
              <span className="w-1 h-1 rounded-full bg-[#D4AF37]/60 animate-bounce" style={{ animationDelay: '0s' }} />
              <span className="w-1 h-1 rounded-full bg-[#D4AF37]/60 animate-bounce" style={{ animationDelay: '0.2s' }} />
              <span className="w-1 h-1 rounded-full bg-[#D4AF37]/60 animate-bounce" style={{ animationDelay: '0.4s' }} />
            </div>
          </div>
        )}
      </div>
      ...
      {/* Interface Port */}
      <div className="p-4 border-t border-neutral-800/60 bg-neutral-950/90 backdrop-blur-3xl">
        <form className="flex gap-3 items-center" onSubmit={handleSend}>
          <div className="flex-1 relative">
            <input
              ...
              placeholder="Message..."
              autoFocus
              className="w-full bg-neutral-900/60 border border-neutral-800/80 rounded-xl px-5 py-3.5 text-[12px] text-white placeholder-neutral-700 focus:outline-none focus:border-[#D4AF37]/50 transition-all font-light"
            />
          </div>
          <button 
            type="submit"
            disabled={isTyping || !inputText.trim()}
            className="w-12 h-12 bg-gradient-to-br from-[#D4AF37] to-[#8B7326] text-black rounded-xl flex items-center justify-center hover:scale-[1.05] active:scale-95 transition-all disabled:opacity-20 group shadow-lg"
          >
